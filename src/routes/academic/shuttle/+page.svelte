<script lang="ts">
  ...
  const filteredVehicles = $derived(
    settings.data.shuttleVehicles.filter(
      (v) => v.plate.includes(searchQuery) || v.model.includes(searchQuery),
    ),
  );
  ...
  const filteredRoutes = $derived(
    settings.data.shuttleRoutes.filter((r) => r.name.includes(searchQuery)),
  );

  const selectedItem = $derived.by(() => {
    if (!selectedId) return null;
    if (activeTab === "vehicles")
      return settings.data.shuttleVehicles.find((v) => v.id === selectedId);
    if (activeTab === "drivers")
      return settings.data.shuttleDrivers.find((d) => d.id === selectedId);
...
<div class="space-y-8 pb-10">
  ...
  <div class="flex gap-6 items-center justify-between">
    ...
    >
      ...
      {#if activeTab === "vehicles"}차량 등록{:else if activeTab === "drivers"}기사
        등록{:else}노선 추가{/if}
    </button>
  </div>
        ...
        <table class="w-full text-left border-collapse whitespace-nowrap">
          ...
          <tbody class="divide-y divide-toss-grey-50/50">
            {#each filteredVehicles as v}
              ...
              >
                <td
                  class="p-6 pl-10 font-black text-[16px] text-toss-grey-700 group-hover:text-toss-blue transition-colors"
                  >{v.plate}</td
                >
                <td class="p-6 font-bold text-toss-grey-500">{v.model}</td>
                <td class="p-6">
                  <span class="font-black text-toss-grey-600"
                    >{settings.data.shuttleDrivers.find(
                      (d) => d.id === v.driverId,
                    )?.name || "미배정"}</span
                  >
                </td>
                <td
                  class="p-6 font-bold text-toss-grey-400 uppercase tracking-tight"
                  >{fmt(v.mileage)} km</td
                >
                <td class="p-6 text-center">
                  <span
                    class="px-3 py-1.5 rounded-full text-[11px] font-black {v.status ===
                    '정상'
            ...
            {#each filteredDrivers as d}
              ...
              >
                ...
                <td class="p-6 text-center">
                  ...
                  {:else}
                    <span
                      class="px-3 py-1.5 rounded-full text-[11px] font-black {d.status ===
                      '운행중'
                        ? 'bg-toss-blue-light text-toss-blue border border-toss-blue/10'
                        : 'bg-toss-grey-50 text-toss-grey-300 border border-toss-grey-100'}"
                    >
                      {d.status}
                    </span>
                  {/if}
          ...
          <tbody class="divide-y divide-toss-grey-50/50">
            {#each filteredRoutes as r}
              ...
              >
                ...
                <td class="p-6">
                  <div
                    class="flex items-center gap-2 font-black text-toss-grey-600"
                  >
                ...
                <td class="p-6 text-center">
                  <span
                    class="px-4 py-1.5 rounded-full bg-toss-blue-light/30 text-toss-blue text-[11px] font-black border border-toss-blue/10"
                    >정상 운행</span
                  >
                </td>
                <td class="p-6 text-right pr-10">
    ...
    <div class="space-y-10" in:fade>
      <section class="grid grid-cols-2 gap-4">
        <div class="p-6 bg-toss-blue-light/30 rounded-[32px] space-y-1">
          ...
          <p class="text-[20px] font-black text-toss-blue">
            {activeTab === "vehicles"
              ? "차량"
              : activeTab === "drivers"
                ? "운전원"
        ...
        <div class="p-6 bg-toss-grey-50 rounded-[32px] space-y-1">
          <p
            class="text-[11px] font-black text-toss-grey-300 uppercase tracking-widest"
          >
            현재 상태
          </p>
          <p class="text-[20px] font-black text-toss-grey-600">
            {(selectedItem as any).status || "정상"}
          </p>
        </div>
      </section>

      <section
        class="bg-toss-grey-25 p-8 rounded-[40px] border border-toss-grey-50 space-y-6"
      >
        <div class="space-y-2">
          <label
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-1">
              ...
              <p class="text-[18px] font-black text-toss-grey-600">
                {fmt((selectedItem as any).mileage)} km
              </p>
            </div>
            <div class="space-y-1">
              <p class="text-[11px] font-bold text-toss-grey-300">
        ...
        {:else if activeTab === "drivers"}
          <div class="space-y-4">
            <div
              class="flex justify-between items-center py-4 border-b border-toss-grey-100"
            >
              <span class="font-bold text-toss-grey-400">연락처</span>
              <span class="font-black text-toss-blue text-[18px]"
                >{(selectedItem as any).phone}</span
              >
            </div>
            <div
              class="flex justify-between items-center py-4 border-b border-toss-grey-100"
            >
              ...
              <span class="font-black text-toss-grey-600"
                >₩{fmt((selectedItem as any).salary || 0)}</span
              >
            </div>
            <div
              class="flex justify-between items-center py-4 border-b border-toss-grey-100"
            >
              ...
              <span
                class="font-black {(selectedItem as any).isOnHoliday
                  ? 'text-orange-500'
                  : 'text-green-500'}"
                >{(selectedItem as any).isOnHoliday ? "휴가 중" : "정상"}</span
              >
            </div>
          </div>
        {/if}
