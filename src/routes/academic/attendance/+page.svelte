<script lang="ts">
  ...
  function getAttendance(studentId: string) {
    return settings.data.attendance.find(
      (a) =>
  ...
  function markAllPresent() {
    ...
    toast.show("전체 출석 처리가 완료되었습니다.", "success");
  }

  const stats = $derived({
    total: classStudents.length,
    present: classStudents.filter((s) => getAttendance(s.id)?.status === "출석")
      .length,
    late: classStudents.filter((s) => getAttendance(s.id)?.status === "지각")
      .length,
    absent: classStudents.filter((s) => getAttendance(s.id)?.status === "결석")
      .length,
  });
</script>
...
<div class="space-y-10 pb-20 max-w-[1600px] mx-auto">
  ...
  <header class="flex justify-between items-end px-2">
    ...
    <div class="flex gap-4">
      <div
        class="flex items-center gap-3 bg-white px-6 py-3 rounded-2xl border border-toss-grey-100 shadow-sm"
      >
        <span
        ...
        <button
          ...
          class="w-12 h-7 rounded-full p-1 transition-all {autoNotify
            ? 'bg-toss-blue'
            : 'bg-toss-grey-200'}"
        >
          <div
            class="w-5 h-5 bg-white rounded-full transition-all {autoNotify
              ? 'translate-x-5'
              : ''}"
          ></div>
        </button>
      ...
      <button
        ...
        class="toss-btn-primary px-8 flex items-center gap-2 shadow-lg shadow-toss-blue/20"
      >
  ...
  <div class="flex gap-8 items-start px-2">
      ...
      <div class="space-y-2">
        {#each settings.data.classes as cls}
          <button
            ...
            class="w-full text-left p-6 rounded-[32px] border transition-all {selectedClassId ===
            cls.id
              ? 'bg-white border-toss-blue shadow-lg ring-4 ring-toss-blue/5'
              : 'bg-white/50 border-toss-grey-100 hover:bg-white hover:border-toss-grey-200'}"
          >
            <p
              class="text-[16px] font-black {selectedClassId === cls.id
                ? 'text-toss-blue'
                : 'text-toss-grey-600'}"
            >
              {cls.name}
    ...
    <main class="flex-1 space-y-6">
      ...
      >
        <div
          class="p-6 border-b border-toss-grey-50 bg-toss-grey-50/30 flex justify-between items-center"
        >
          <div class="relative w-80 group flex items-center">
            ...
            <input
              ...
              class="w-full h-[60px] pl-16 pr-6 bg-white rounded-2xl border border-toss-grey-100 outline-none font-bold text-[16px] focus:border-toss-blue transition-all shadow-sm"
            />
          </div>
          <div
            class="flex items-center gap-2 text-[13px] font-bold text-toss-grey-400"
          >
            <Calendar size={16} />
            {today} 기준
          </div>
              ...
              <tr class="hover:bg-toss-grey-50 transition-all group">
                <td class="py-2.5 px-8">
                  <div class="flex items-center gap-3">
                    <div
                      class="w-8 h-8 rounded-lg bg-toss-grey-100 flex items-center justify-center font-black text-[14px] text-toss-grey-500"
                    >
                      {s.name[0]}
                    </div>
                    <span
                ...
                <td
                  class="py-2.5 px-8 text-[13px] font-bold text-toss-grey-400"
                >
                  {s.school} · {s.grade}
                </td>
                <td class="py-2.5 px-8">
                  <div class="flex gap-1">
                    {#each ["출석", "지각", "결석", "조퇴"] as status}
                      <button
                        ...
                        class="px-3 py-1.5 rounded-lg text-[11px] font-black transition-all {att?.status ===
                        status
                          ? status === '출석'
                          ...
                          : 'bg-toss-grey-50 text-toss-grey-300 hover:bg-toss-grey-100 hover:text-toss-grey-500'}"
                      >
                        {status}
                      </button>
                    {/each}
                ...
                <td class="py-2.5 px-8 text-right">
                  {#if att}
                    ...
                    >
                  {:else}
                    <span class="text-[12px] font-bold text-toss-grey-200"
                      >-</span
                    >
                  {/if}
                </td>
                <td class="py-2.5 px-8 text-right">
                  <button
                    class="p-1.5 text-toss-grey-200 hover:text-toss-blue transition-colors"
                    title="문자 강제 발송"
                  >
                    <Smartphone size={16} />
                  </button>
